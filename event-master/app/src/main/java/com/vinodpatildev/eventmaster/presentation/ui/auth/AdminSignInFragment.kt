...
import com.vinodpatildev.eventmaster.databinding.FragmentAdminSignInBinding
...
class AdminSignInFragment : Fragment() {
    ...
    companion object {
        fun newInstance() = AdminSignInFragment()
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        progressDialog.setMessage("Signing as Admin....")
        ...
        binding?.btnLogin?.setOnClickListener {
            if (binding?.etUsernameLogin?.text?.trim().toString().isEmpty()) {
                ...
                viewModel.signInAdmin(username,password)
            }
        }
...
//            parentActivity.finish()
        ...
        viewModel.signInResultAdmin.observe(viewLifecycleOwner, Observer { response->
            when(response){
                is Resource.Success->{
                    progressDialog.hide()
                    response.data?.let {
